...
class dlnet:
    ...
    def nloss(self,y, yh):
        ...
        '''
        return np.mean(np.square(y - yh)) / 2
    ...
    def backward(self, y, yh):
        ...
        dL_u2 = (yh - y) * self.dTanh(self.ch['u2'])

        dLoss_theta2 = np.mean(self.ch['o1'] * dL_u2, axis=1)

        dLoss_b2 = np.mean(dL_u2)

        dL_o1 = dL_u2 * self.param['theta2'].T

        do1_u1 = self.dRelu(self.ch["u1"])

        dL_u1 = dL_o1 * do1_u1

        dLoss_theta1 = np.mean(dL_u1[:, np.newaxis, :] * (self.ch["X"]), axis=2)

        dLoss_b1 = np.mean(dL_u1, axis=1)[:, np.newaxis]
    ...
    def gradient_descent(self, x, y, iter = 60000):
        ...
        for i in range(iter):
            ...
            if i % 1000 == 0:
                ...
                print("Loss after iter "+ str(i)+ ":" + str(loss))
    ...
    def batch_gradient_descent(self, x, y, iter = 60000):
        ...
        index = 0
        while count < iter:
            if(index + self.batch_size -1 > x.shape[1]):
                xi1 = x[:, index: x.shape[1]]
                yi1 = y[:, index: x.shape[1]]
                index = self.batch_size - (x.shape[1] - index) 
                xi2 = x[:, 0: index]
                yi2 = y[:, 0: index]
                xi = np.concatenate((xi1, xi2), axis = 1)
                yi = np.concatenate((yi1, yi2), axis = 1)
            else:
                xi = x[:, index: index + self.batch_size]
                yi = y[:, index: index + self.batch_size]
                index += self.batch_size

            yhi = self.forward(xi)
            self.backward(yi, yhi)
            if count % 1000 == 0:
                loss = self.nloss(yi, yhi)
                print("Loss after iter "+ str(count)+ " :" + str(loss))
                self.loss.append(loss)
            count += 1
