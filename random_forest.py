...
class RandomForest(object):
    ...
    def _bootstrapping(self, num_training, num_features, random_seed = None):
        ...
        row_idx = np.random.choice(num_training, num_training, True)  
        #################################################################################

        ############# Get Col Indices Second - write your code below ####################
        col_idx = np.random.choice(num_features, int(num_features * self.max_features), False)
        ##################################################################################
    ...
    def plot_feature_importance(self, data_train):
        ...
        for i in range(self.n_estimators):
            ...
            ax.set_title('Decision tree: ' + str(i))
            tree_i = self.decision_trees[i]
            label = data_train.columns[self.feature_indices[i]]
            feature_importance = tree_i.feature_importances_
            combined = np.vstack((label, feature_importance)).T
            sortedArr = combined[combined[:,1].argsort()]
            sortedArr = sortedArr[::-1]
            label, feature_importance = zip(*sortedArr)
            label = np.asarray(label)
            feature_importance = np.asarray(feature_importance)
            ax.bar(label, feature_importance)
        plt.show()
